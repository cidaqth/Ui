...
Main.DoYourThing = function(chr) -- The main thing, connects to CharacterAdded and then executes on your character (if it exists)
    ...
    task.wait(1.5) -- Just in case, wait 3 seconds before actually starting the repeat loop
    repeat
        ...
        task.wait(0.1) -- Just in case, wait a moment before buying
        Main:Shop(buy, true) -- Buy the item
        task.wait(3.1) -- Wait 10 seconds, because if you do it quicker than that the game will not let you sell (the added .5 is a failsafe)
        ncf = seller.CFrame -- Loop tp to the trader you can sell to
        task.wait(0.01) -- Just in case, wait a moment before selling
        Main:Shop(buy, false) -- Sell the item
        task.wait(0.01) -- Wait a moment just in case it doesn't sell before tping away
